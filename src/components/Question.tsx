...
export default function Question(question: TPhilosophyQuestion) {
  ...
  const { answers, hasBeenChecked, expandedQuestion } = useAppSelector(state => state.questions);
  ...
  return (
    ...
    >
      ...
      >
        <Chip label={question.id} color={getChipColor()} size='medium' sx={{ fontWeight: 'bold' }} />
        <Typography
          variant='h6'
          sx={{
            fontWeight: 600,
      ...
      <AccordionDetails sx={{ px: { xs: 2, sm: 3 }, pb: { xs: 2, sm: 3 } }}>
        ...
        <RadioAnswers selectedValue={selectedAnswer} onChange={handleAnswerChange} />
      </AccordionDetails>
    </Accordion>
  );
}
